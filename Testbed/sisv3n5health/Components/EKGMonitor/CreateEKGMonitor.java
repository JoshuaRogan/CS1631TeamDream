...
public class CreateEKGMonitor
{
	...
	static void ProcessMsg(KeyValueList kvList) throws Exception
	{
		...
		Below are the sending messages' attributes list:
			MsgName: EKG data analysis result	MsgID: 1007	Attrs: EKGDataResult; EKGData
		For more information about KeyValueList, read comments in Util.java.
		****************************************************/
		case 1008:
			System.out.println("Message MsgName:EKG data MsgID:1008 received, start processing.");
			/*************************************************
			Add code below to process Message MsgName:EKG data MsgID:1008
			This message has following attributes: EKGData, use KeyValueList.getValue(String key) to get the values.
			If needed, don't forget to send a msg after processing. See previous comments on how to send a message.
			*************************************************/

			KeyValueList msg = new KeyValueList();
			msg.addPair("MsgID","1007");
			msg.addPair("Description","EKG data result analysis");
			msg.addPair("EKGData","Value");
			msg.addPair("EKGResult","Value");
