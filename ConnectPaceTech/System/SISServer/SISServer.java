...
{	
	...
	public static ArrayList<Socket> FindActiveSocketCSM(String in)
	{
		ArrayList<Socket> results = new ArrayList<Socket>();
		for(int i=0; i<csm.size(); i++)
			if(csm.get(i).compname.equals(in) && csm.get(i).active)
				results.add(csm.get(i).compsocket);
		if(results.size() == 0)
			return null;
		return results;
	...
	/*Eliminate comps by comp name*/
	public static void EliminateComp(String in) throws Exception
	{
		PrintStream printOut;
		
		for(int i=0; i<cmc.size(); i++)
		{
			if(cmc.get(i).compname.equals(in))
			{
				cmc.remove(i);
				break;
			}
		...
		for(int i=0; i<csm.size(); i++)
			if(csm.get(i).compname.equals(in))
			{
				try {
					printOut= new PrintStream(csm.get(i).compsocket.getOutputStream());
					printOut.println("MsgID$$$22");
					csm.get(i).compsocket.close();
					csm.remove(i);
					i--;
				} catch (IOException e) {
					System.out.println("Component "+csm.get(i).compname+" connection error!");
	...
	public static void EliminateAll()throws Exception
	{
		int i=0;
		while(cmc.size()>i)
		{
			if(cmc.get(i).compname.contains("DataSender")||cmc.get(i).compname.contains("Eliminator")||cmc.get(i).compname.contains("Verifier")||cmc.get(i).compname.contains("Enumerator")||cmc.get(i).compname.contains("TimeController")||cmc.get(i).compname.contains("WebInterface")||cmc.get(i).compname.contains("SISSIAUI")||cmc.get(i).compname.contains("DataFilter"))
			{
				i++;
				continue;
			}
			EliminateComp(cmc.get(i).compname);
		}
	}
	
	public static void EliminateAllwithSysComps()throws Exception
	{
		while(cmc.size()>0)
		{
			EliminateComp(cmc.get(0).compname);
		}
	}
		...
		public void run()
		{
			try
			{
				...
				do
				{
					...
					int cl_msgid = Integer.parseInt(kvInput.getValue("MsgID"));
					if( cl_msgid != 23 && cl_msgid != 26)
					{
						if(cl_msgid == 22)
							EliminateComp(kvInput.getValue("Name"));
						else if(cl_msgid == 24)
							TurnOn(kvInput.getValue("Name"));
						else if(cl_msgid == 25)
							TurnOff(kvInput.getValue("Name"));
						else
